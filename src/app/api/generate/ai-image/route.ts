...
//
// 예전에는 Leonardo 전용이었는데 그 키가 없어 화면이 계속 "준비 중"이었다.
// 카드뉴스에서 이미 쓰고 있는 Gemini로 돌린다 — 새 키가 필요 없다.
// Leonardo는 Gemini 키가 아예 없을 때의 대체 경로로만 남겨둔다.

// 노트 카드 라우트와 같은 상한. 사진 한 장에 80~90초쯤 걸린다.
...
- 한국의 풍경·건물 맥락에 맞게 그릴 것`;
  ...
  // 남은 예산 안에서만 기다린다. 모델마다 무작정 기다리면 두 번 시도에
  // 함수 제한을 넘겨 504로 죽는다 — 실제로 그렇게 됐다.
